    ...
    if (typeof module !== 'undefined' && module.exports) {
            ...
            return function(err, response, body) {
                ...
                if (response.statusCode >= 200 && response.statusCode < 300){
                    if (TOTAL_COUNT_HEADER in response.headers) {
                        body.totalCount = parseInt(response.headers[TOTAL_COUNT_HEADER]);
                    }
                    callback(err, body);
                } else {
        ...
        request = function(opts, callback) {
            ...
            if ('withCredentials' in xhr) {
                ...
                if (opts['auth']) {
                    xhr.open(opts['method'], url, true, opts['auth']['user'], opts['auth']['pass']);
                } else {
                    ...
                    if (xhr.readyState == 4) {
                        if (xhr.status >= 200 && xhr.status < 300) {
                            ...
                            if (xhr.getResponseHeader(TOTAL_COUNT_HEADER)) {
                                res.totalCount = parseInt(xhr.getResponseHeader(TOTAL_COUNT_HEADER));
                            }
                            callback(null, res);
                        } else {
                            if (xhr.responseText) {
